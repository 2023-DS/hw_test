...
function test_case() {
    ...
    /bin/timeout 1 ${PWD}/output/${hwId}_${proId}.output < ${test_dir}/input_${caseId}.txt >> ${PWD}/tmp_A.txt
    ret=$?
    if [ $ret -ne 0 ]; then
        /bin/echo "${hwId} ${proId} input_${caseId}.txt RE/TLE"
    else
        /bin/cat "${test_dir}/output_${caseId}.txt" > ${PWD}/tmp_B.txt
        /bin/diff -w ${PWD}/tmp_A.txt ${PWD}/tmp_B.txt > /dev/null
        ret=$?
...
function test_all() {
    ...
    do
        hwId=$(/bin/echo $dirname | /bin/awk -F '_' '{print $1}')
        proId=$(/bin/echo $dirname | /bin/awk -F '_' '{print $2}')
        for filename in $(/bin/ls ${PWD}/test/${dirname})
        do
            caseId=$(/bin/echo $filename | /bin/awk -F 'input_|.txt' '{print $2}')
            if [ -n "$caseId" ]; then
